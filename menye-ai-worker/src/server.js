...
function getReferenceStylePrompt(slotId, options = {}) {
  ...
  const hasLockDetail = !!options.hasLockDetail;
  const backgroundLockApplyDescriptionHint = hasLockDetail
    ? 'applyDescription 还必须强调保持第一张整门图的门体、包边、把手、玻璃、颜色、材质和门款不被重画；锁体/智能锁由 lock-detail 参考图执行替换，不能在背景任务里要求保留原锁不变；只做抠图贴合、透视对齐、阴影和光线融合。'
  ...
  switch (slotId) {
    ...
    case 'lock-detail':
      return [
        ...
        '小圆孔位置规则：小圆孔是实体应急锁孔，用于智能锁损坏后开门；必须按参考图中它相对智能锁面板、锁体底座、把手和门缝的真实位置识别。如果参考图的小圆孔在锁体/底座/面板范围内，最终也必须贴合在新锁本体范围内，不能在锁体外的门板上额外重复生成独立圆孔；只有参考图明确显示它是锁体外单独安装件时，才允许作为独立小圆孔。',
        ...
        '如果 lockIntegrationType 是 handle-integrated，applyDescription 必须写明：把智能锁面板和一体式把手作为不可拆整体融合；如果参考图是双把手，最终必须生成双把手，不能变成单把手；把手长度、宽窄、面板高度必须接近参考图；小圆孔按参考图真实相对位置保留，若在锁体/底座/面板范围内则必须融合在新锁本体范围内，不能在门板上额外重复画一个独立圆孔；仍不得改变门型结构、门板线条、包边和背景。',
        '不要解释，不要输出 markdown。'
      ].join('\n');
    case 'panel-style-detail':
      return [
    ...
    case 'background-reference':
      return [
        ...
        backgroundLockApplyDescriptionHint,
        '不要解释，不要输出 markdown。'
      ].join('\n');
    default:
      return '';
...
function buildDoorImageInstruction(job, maskBox, handleStyle, referenceStyles, dimensionBoxes) {
  ...
  const allowHandleOutwardRelocation = hasLockDetail && userExplicitlyPreservesHandles && isMultiLeafDoorType;
  ...
  const backgroundLockPolicyText = hasLockDetail
    ? (userExplicitlyPreservesHandles
      ? '锁体/智能锁按 lock-detail 参考图执行替换/融合；客户已明确保留原把手/拉手，因此只能迁移智能锁面板、锁芯、密码/指纹区、应急小圆孔和必要安装底座，不得迁移参考图把手本体。双开/多开门若中缝空间不足，应把原左右拉手分别水平向外侧小幅平移给智能锁让位；必须保留拉手数量、长度、颜色、造型和竖向高度，不得截断、缩短或删除。'
  ...
  const imageLines = orderedReferenceImages.map((item, index) => {
    ...
    if (hasBackgroundReference && item.slotId === 'full-door') {
      return `输入图${index + 1}：${label}。这是门体、门型、比例、透视和未被局部参考图替换的原五金来源；${backgroundHandlePolicyText}；${backgroundLockPolicyText}；最终必须从这张图中抠出门体，贴入输入图1背景参考图的目标门位。`;
    }
    if (index === 0) {
  ...
  const lockDetailMustApplyInstruction = hasLockDetail
    ? [
        ...
        '门扇旧孔清零规则：执行锁体/智能锁替换后，门扇上允许保留的五金只有新智能锁、合页、以及客户明确要求保留的把手/拉手；原图门扇上的独立小黑方块、方形按钮、小圆猫眼、中心旧圆孔、旧门铃、旧锁芯和旧锁面板都必须用门面颜色和纹理修复掉。除非客户明确说保留猫眼/门铃/旧孔，否则最终图中不能在新智能锁之外再出现这些旧五金点。',
        userExplicitlyPreservesHandles
          ? '客户明确要求保留把手/拉手：即使 lock-detail 被识别为把手一体式智能锁，也不得迁移参考图中的把手本体；只能把智能锁面板、密码/指纹/刷卡区、锁芯孔、小圆孔和必要底座作为独立锁具元素融合到五金安装区。双开/多开门中缝空间不足时，应让原左右拉手分别水平向外侧小幅平移，为智能锁留出中间安装位；保留原把手/双拉手的长度、数量、形状、颜色、材质和竖向高度。'
          : '',
        userExplicitlyPreservesHandles
          ? '保留把手优先级：客户明确保留把手/拉手时，保留的是把手款式、长度、颜色、数量、成对关系和竖向高度，不是必须原地不动。如果智能锁与把手空间冲突，优先把左右拉手各自向外侧平移，给中间智能锁留位置；不得覆盖、截断、分裂、缩短、改色或删除任何一根原把手/拉手。'
        ...
        userExplicitlyPreservesHandles
          ? '双开门让位规则：如果客户要求保留双拉手，最终左右两根原拉手必须保持完整且成对，金色竖拉手必须是连续完整的长条；为了给中缝智能锁留位置，可以把左拉手向左移、右拉手向右移，移动后仍保持相同高度、长度、颜色、造型和成对关系。不得把任一拉手变短、断开、下移、错位或改成半截。'
        ...
        '智能锁验收失败定义：最终图中没有清晰可见的新智能锁面板、密码/指纹/锁孔等参考图核心锁具特征，或者只保留原来的金色双拉手/原五金不变，或者新智能锁已经生成但门扇上仍残留原图独立小黑方块、小圆猫眼/旧圆孔、旧锁面板等旧五金点，都属于失败。'
      ].filter(Boolean).join('\n')
    : '';
  const multiLeafDoorLockInstruction = isMultiLeafDoorType
    ? [
        ...
        doorType === '双开门'
          ? (allowHandleOutwardRelocation
            ? '双开门必须保留左右两扇门和中缝位置；本次为了安装中缝智能锁，允许左右双拉手分别水平向外侧小幅平移，但不得改变左右门扇比例、左右对称关系和中缝收口。'
  ...
  const lockReferenceCanReplaceHandle = lockReferenceIsHandleIntegrated && !userExplicitlyPreservesHandles;
  ...
  const requiredReferenceTasks = [
    ...
    hasLockDetail ? (lockReferenceCanReplaceHandle
      ? `锁体/智能锁：必须按锁体/智能锁细节图替换为一体式把手锁整体，包含智能锁面板、指纹/密码/刷卡区和与其物理一体的把手；小圆孔/锁芯孔必须按参考图真实相对位置贴合到新锁本体或其明确安装底座上，不能在新锁下方或门板空白处额外重复画独立圆孔；${lockReferenceIsDoubleHandle ? '参考图是双把手，最终必须生成双把手，不能只生成单把手；' : ''}不能保留整门图里的原智能锁或原把手不变`
      : (userExplicitlyPreservesHandles
        ? '锁体/智能锁：客户明确要求保留原把手/拉手，因此只迁移智能锁面板、密码/指纹/刷卡区、锁芯孔、小圆孔和必要安装底座；不得迁移参考图中的一体式把手本体。双开/多开门允许把原拉手水平向外侧平移给智能锁让位，但不得截断、删除或缩短原把手/拉手。'
  ...
  const requiredReferenceTaskInstruction = requiredReferenceTasks.length
    ? [
        ...
        hasEdgeTrimDetail
          ...
          : '',
        hasLockDetail
          ? '智能锁验收标准：只要上传了 lock-detail，最终图中必须清晰出现新智能锁面板、密码/指纹/锁孔/小圆孔等参考图核心特征；不能因为背景、颜色、白底、把手或其他局部任务而跳过智能锁，也不能只保留原来的拉手或原锁具不变。'
          : ''
      ].filter(Boolean).join('\n')
    : '';
  ...
  const layeredTaskOrderInstruction = requiredReferenceTasks.length
    ? [
        hasColorSample
          ? (allowHandleOutwardRelocation
            ? '强制执行顺序：先锁定第一张整门图的门型几何、门扇比例、线条位置和把手款式/竖向高度，再按锁体任务将左右拉手分别水平向外侧平移给中缝智能锁让位，再按已上传局部参考图分别处理对应局部层（包边、锁体、门板造型、气窗、门头/门柱、材质纹理等），再按颜色参考图处理门体颜色层，最后才处理背景/白底。'
            : '强制执行顺序：先锁定第一张整门图的门型几何、门扇比例、线条位置和把手位置，再按已上传局部参考图分别处理对应局部层（包边、锁体、门板造型、气窗、门头/门柱、材质纹理等），再按颜色参考图处理门体颜色层，最后才处理背景/白底。')
          : (allowHandleOutwardRelocation
            ? '强制执行顺序：先锁定第一张整门图的门型几何、门扇比例、线条位置、门体原始颜色和把手款式/竖向高度，再按锁体任务将左右拉手分别水平向外侧平移给中缝智能锁让位，再按已上传局部参考图分别处理对应局部层（包边、锁体、门板造型、气窗、门头/门柱、材质纹理等），最后才处理背景/白底；本次没有颜色参考图，不存在门体颜色层。'
        ...
        hasLockDetail
          ? '最终自检：只要上传 lock-detail，成图中锁体/智能锁安装区必须有清晰可见的新锁具核心特征；如果背景已替换、颜色已改变或包边/把手已处理，但智能锁仍是原图旧锁、旧拉手或完全没有新增智能锁面板，则整张图仍视为失败。'
          : ''
      ].join('\n')
    : '';
  ...
  const singleDoorAdditionalPartInstruction = [
    hasLockDetail
      ? [
          ...
          lockReferenceCanReplaceHandle
            ? `系统识别到该智能锁与把手为物理一体式结构，因此本次锁体任务必须把智能锁面板和一体式把手作为同一个不可拆整体替换到整门图上；把手数量、长度、宽窄、颜色和面板位置应随一体式锁具一起保持一致。只能替换与该一体式智能锁直接冲突的原智能锁/原把手；双开门、子母门或多开门中未被智能锁占用的另一侧拉手必须保留。小圆孔/锁芯孔不属于把手本体，但必须按参考图真实相对位置融合到新锁本体或其明确安装底座上，不能在新锁外的门板空白处额外重复生成。${lockReferenceIsDoubleHandle ? '参考图为双把手结构，必须在最终图中保留两根对应把手及其相对位置；不能减少成单把手。' : ''}`
            : (userExplicitlyPreservesHandles
              ? '客户已明确保留把手/拉手，因此即使锁体参考图被识别为把手一体式智能锁，也只能提取智能锁面板、密码/指纹/刷卡区、锁芯孔、小圆孔和必要底座；参考图中的把手本体不得迁移。新锁需要占用中缝五金区时，应把原把手/双拉手水平向外侧小幅平移给锁留位置；必须保留原把手/双拉手的完整长度、数量、颜色、材质、形状和竖向高度。'
          ...
          lockStyle && lockStyle.hasRoundHole
            ? `智能锁参考图包含小圆孔，最终图必须保留真实实体应急锁孔细节；${lockStyle.roundHoleDescription || '小圆孔应位于智能锁本体或其明确安装底座附近的合理位置'}；相对位置=${lockStyle.roundHoleRelativePosition || '按参考图相对智能面板、把手和门缝的位置'}。小圆孔必须保持参考图中相对智能锁本体的位置：如果参考图里它在锁体/底座/面板范围内，最终也必须贴合在新锁本体范围内；不能在新锁下方、旁边门板空白处或错误门扇上额外重复生成独立圆孔。`
          ...
          lockReferenceCanReplaceHandle
            ...
            : (userExplicitlyPreservesHandles
              ? '非一体式锁体/智能锁修改不得改变门扇外轮廓、门板线条数量、门型比例、包边结构和背景；客户明确保留把手/拉手时，锁具安装区必须有清晰可见的新智能锁结果，但只能覆盖旧锁具、旧锁孔、旧小面板和非把手残留，不能覆盖任何原把手/拉手本体或把手底座。'
  ...
  const additionalPartConflictResolutionInstruction = [
    ...
    hasLockDetail
      ? (lockReferenceCanReplaceHandle
        ...
        : (userExplicitlyPreservesHandles
          ? '冲突消解：客户明确保留把手/拉手时，锁体/智能锁参考图只打开锁具五金局部变化权限；即使参考图是一体式把手锁，也不得迁移把手本体，不得覆盖、截断、缩短、分裂或删除原把手/双拉手。若智能锁与拉手重叠，双开/多开门优先将左右拉手分别向外侧水平平移给智能锁留位，同时保持拉手完整。'
  ...
  const immutableBaseDoorInstruction = [
    ...
    hasBackgroundReference
      ? `第一张整门图不是最终底图，而是门体抠图来源；背景参考图才是最终底图。必须保留第一张图的门型、颜色、包边、玻璃和材质；${backgroundHandlePolicyText}；${backgroundLockPolicyText}；同时保留背景参考图的墙面、地面、家具、装饰和空间构图。`
    ...
    allowHandleOutwardRelocation
      ? '把手位置让位例外：上述“把手安装位置保持”不禁止本次双开/多开门左右拉手为了中缝智能锁安装而分别水平向外侧小幅平移；外移后必须保持原拉手完整长度、颜色、款式、数量和竖向高度。'
      : '',
    lockReferenceCanReplaceHandle
    ...
    hasPanelStyleDetail || hasGlassGrilleDetail || hasHeaderColumnDetail
      ? (allowHandleOutwardRelocation
        ? '失败结果定义：除门板线条/造型参考图、气窗参考图、门头/门柱参考图明确对应的目标区域，以及左右拉手为中缝智能锁让位的水平外移外，最终图只要出现新的门扇比例、无关的新把手位置、新的锁体位置、无关区域的新门板线条、无关区域的新玻璃位置、门体颜色被气窗或门头/门柱参考图带偏、或变成另一扇浅色/深色商品门，就属于失败，必须改回第一张整门图的基础门型和原门体颜色。'
      ...
      : (allowHandleOutwardRelocation
        ? '失败结果定义：除左右拉手为中缝智能锁让位的水平外移外，最终图只要出现新的门板线条数量、新的线条位置、新的浮雕/门芯图案、新的门扇比例、无关的新把手位置、或变成另一扇浅色/深色商品门，就属于失败，必须改回第一张整门图的门型。'
  ...
  const doorIdentityLockInstruction = [
    ...
    allowHandleOutwardRelocation
      ? '门型锁定例外：本次为了给中缝智能锁留安装位，左右双拉手可以分别水平向外侧小幅平移；这不是换把手款式，也不是破坏门型。'
      : '',
    lockReferenceCanReplaceHandle
      ? `把手一体式智能锁例外：只允许在一体式智能锁把手自身区域内替换把手和锁面板的整体位置、数量、长度、宽窄、颜色、黑色面板和安装衔接；允许为保持智能面板、把手和实体应急锁孔的相对位置而小幅移动整个一体式把手锁；小圆孔/锁芯孔必须贴合参考图中相对新锁本体的位置，不能在锁体外门板空白处重复生成第二个独立圆孔；${lockReferenceIsDoubleHandle ? '参考图是双把手时，最终必须保留双把手数量、左右/双扇相对位置和长短比例，不能生成单把手。' : ''}如果整门图中已有隐蔽智能锁、小圆孔或旧锁面板，必须被新的一体式智能锁覆盖或清理，不得叠加残留。门型、包边、门板线条和背景仍冻结。`
      : '',
    edgeTrimColorProtectedFromColorSample
    ...
    hasPanelStyleDetail || hasGlassGrilleDetail || hasHeaderColumnDetail
      ? (allowHandleOutwardRelocation
        ? '如果最终图在目标区域以外的门板线条数量、线条位置、门芯造型、门扇比例、非让位性质的把手位置、锁体位置、开门方向或门框比例与第一张整门图明显不同，应视为失败结果，必须改回第一张整门图的基础门型。'
        : '如果最终图在目标区域以外的门板线条数量、线条位置、门芯造型、门扇比例、把手位置、锁体位置、开门方向或门框比例与第一张整门图明显不同，应视为失败结果，必须改回第一张整门图的基础门型。')
      : (allowHandleOutwardRelocation
        ? '如果最终图的门板线条数量、线条位置、门芯造型、门扇比例、非让位性质的把手位置、开门方向或门框比例与第一张整门图明显不同，应视为失败结果，必须改回第一张整门图的门型结构。'
  ...
  const backgroundInstruction = useDefaultWhiteBoardBackground
    ...
    : hasBackgroundReference
      ? [
          ...
          `换背景时必须保留第一张整门图中的门体、门扇外轮廓、门框比例、包边、玻璃、门体颜色、材质纹理和已经执行的其他局部部件任务；${backgroundHandlePolicyText}；${backgroundLockPolicyText}；允许的几何变化只限于为了贴合背景门位所需的整体缩放、透视拉伸、旋转和轻微裁切。`,
  ...
  const frontDimensionSpanInstruction = isDimensionAnnotationTask && dimensionAnnotationData.viewSide === 'front'
    ? (dimensionAnnotationData.hasDoorOpeningRequest && dimensionAnnotationData.hasVisibleOpeningRequest
      ? '正面图门洞/见光尺寸取线规则：客户同时要求标注门洞尺寸和见光尺寸时，门洞尺寸取包边厚度中线/半包边位置，横向宽度必须形成三层清晰边界：含包边宽标最外侧包边外沿；门洞宽标包边厚度中线/半包边位置；见光宽标不含包边的净可见开口边界。竖向高度的底部如果没有下槛、气窗、门头或其他额外部件，应共用门底同一个下边界；不要为了分层把底部画到不同位置。竖向高度只通过上边界区分：含包边高的上边界取最外侧上包边外沿，门洞高的上边界取上包边厚度中线/半包边位置，见光高的上边界取不含包边的净可见开口上边界。'
  ...
  const backgroundInputOrderInstruction = hasBackgroundReference
    ? [
        maskBox
          ? `输入顺序强约束：本次为了启用背景门位 mask，输入图1是背景参考图，也是最终画布底图；输入图2是整门上下文图，也是门体/门型/门色和未被局部参考图替换的原五金来源；${backgroundHandlePolicyText}；${backgroundLockPolicyText}。`
          : `输入顺序强约束：输入图1是背景参考图，也是最终画布底图；输入图2是整门上下文图，也是门体/门型/门色和未被局部参考图替换的原五金来源；${backgroundHandlePolicyText}；${backgroundLockPolicyText}。`,
        ...
        hasLockDetail
          ? '最终结果应等于：输入图1背景底图 + 输入图2整门抠图贴入输入图1的 mask 门位区域 + lock-detail 智能锁替换任务真实落到门上；不是重新画一张室内效果图，也不是只贴门不换锁。'
  ...
  return [
    ...
    allowHandleOutwardRelocation
      ? '禁止从零生成新门款；禁止把其他参考图中的整门样式迁移到第一张整门图上；禁止把第一张整门图替换成看起来相似但线条、比例或门芯不同的新门。左右拉手为了中缝智能锁让位而水平外移是本次允许的局部五金调整，不属于换门款。'
    ...
    !hasHandleDetail && !hasEdgeTrimDetail && !hasColorSample && !hasLockDetail && !hasPanelStyleDetail && !hasGlassGrilleDetail && !hasHeaderColumnDetail && !hasTextureReference && !hasBackgroundReference
      ...
      : !hasHandleDetail
        ? (lockReferenceCanReplaceHandle
          ...
          : (userExplicitlyPreservesHandles
            ? '当前没有门把手细节照，且客户明确要求保留把手/拉手；请完整保留整门图中的现有把手/拉手数量、长度、形状、颜色和底座。双开/多开门为了给智能锁留中缝安装位，可以把左右拉手分别水平向外侧小幅平移，但不得把参考图中的把手本体迁移过来，也不得让新智能锁遮挡、截断或缩短任何原把手/拉手。'
