...
function buildDoorImageInstruction(job, maskBox, handleStyle, referenceStyles) {
  ...
  const hasEdgeTrimOnlyReference = hasEdgeTrimDetail && !hasHandleDetail && !allowDoorSurfaceColorChange && !allowBackgroundChange;
  const edgeTrimScopeLimitInstruction = allowDoorSurfaceColorChange
    ? '最高优先级限制：包边替换不是整门换款。包边任务只允许修改包边、门套线、收口条、压线和其极小衔接边缘；整门可见门面颜色默认按颜色参考图统一执行，但补充要求或独立部件参考图指定的不同颜色优先；门扇主体造型、门板花纹、门板线条数量、线条位置、门型比例、玻璃、门芯结构、五金把手必须保持整门照原样。严禁为了适配包边而重画门扇。'
    : '最高优先级限制：包边替换不是整门换款。只允许修改包边、门套线、收口条、压线和其极小衔接边缘；门扇主体、门板花纹、门板线条数量、线条位置、门型比例、玻璃、门芯造型、门面颜色、五金把手必须保持整门照原样。严禁为了适配包边而重画门扇。';
  const requiredReferenceTasks = [
    hasHandleDetail ? '门把手：必须按门把手细节图融合/替换' : '',
    hasEdgeTrimDetail ? '包边：必须识别包边参考图中的包边并融合/替换' : '',
    hasColorSample ? '整门颜色：默认必须按颜色参考图统一调整整门可见门面颜色和材质观感；如补充要求或部件参考图指定不同颜色，则按指定部件优先' : ''
  ].filter(Boolean);
  ...
  const layeredTaskOrderInstruction = requiredReferenceTasks.length
    ? [
        '强制执行顺序：先锁定第一张整门图的门型几何、门扇比例、线条位置和把手位置，再按包边参考图处理包边层，再按颜色参考图处理整门统一颜色层，最后才处理背景/白底。',
        '后执行的任务不能覆盖先执行的任务：颜色任务默认统一整门可见门面颜色，但不能覆盖用户补充要求里点名的不同颜色，也不能覆盖已有独立参考图约束的部件颜色；背景/白底任务不能删除、变浅、简化或重画包边。',
  ...
  const edgeTrimStrictInstruction = hasEdgeTrimDetail
    ? [
        ...
        edgeTrimStyle && edgeTrimStyle.applyDescription
          ...
          : '',
        '最终成图中的包边必须优先保持包边参考图中“门洞周围包边区域”的颜色、材质、表面质感、宽窄比例、截面层次、凹凸倒角、收边方式、线条、纹理和关键装饰细节。',
        allowEdgeTrimColorChange
          ? '用户这次明确要求调整包边颜色，因此包边颜色可以按用户要求改变；但包边材质观感、宽窄比例、截面层次、线条结构和关键细节仍应保持与包边参考图一致。'
          : '不要因为门体颜色、环境光或整体风格自动改变包边颜色；包边颜色必须优先跟随包边参考图中包边区域的固有颜色。',
  ...
  const auxiliaryReferenceInstruction = [
    hasEdgeTrimDetail
      ...
      : '',
    hasColorSample
      ? '高优先级指令：输入中包含颜色参考图时，AI 必须自动识别主色、色偏、纹理和表面质感，并默认按该颜色参考图统一调整整门可见门面颜色；不需要客户额外说明“改成这个颜色”。如果补充要求明确写了不同部件不同颜色，或某个部件有独立参考图，则该部件按更具体的要求优先。'
      : '',
    hasEdgeTrimDetail || hasColorSample
      ? '包边参考图和颜色参考图只用于约束对应部件；除非用户明确要求，不要因为这些参考图顺带改变门把手、门型结构、门扇样式、背景或其他未点名内容。多张参考图同时存在时，必须像图层编辑一样分别执行：包边层只改包边，颜色层默认统一整门可见门面颜色，背景层只改背景；如果用户或独立参考图给某个部件指定了不同颜色，则该部件不要被统一颜色覆盖。'
      : ''
  ].filter(Boolean).join('\n');
  const colorSampleStrictInstruction = colorSampleStyle
    ? [
        ...
        '优先级例外：如果补充要求明确指定某个部件使用不同颜色，或该部件上传了独立参考图（例如包边参考图、门把手细节图），则该部件按更具体的补充要求或独立参考图执行，不被整门统一颜色覆盖。',
        ...
        '请忽略颜色参考图中的拍摄光照、阴影、高光、反光和白平衡偏差，提取材料本身的固有颜色后应用到整门可见门面。',
        ...
        hasEdgeTrimDetail
          ? '颜色任务默认允许统一调整整门可见门面颜色、纹理色差和必要材质观感；但包边已由包边参考图单独约束时，包边颜色和材质按包边参考图执行，不被颜色参考图覆盖；门型结构、门板线条数量、线条位置、凹凸深浅、玻璃、把手和门框比例必须保持整门照原样。'
  ...
  const structuredReferenceInstruction = [
    ...
    hasEdgeTrimDetail
      ...
      : '',
    hasColorSample
      ? '结构化需求确认：客户上传了颜色参考图，因此“默认按颜色参考统一整门可见门面颜色和材质观感”本身已经是明确需求；如补充要求指定局部不同颜色，则局部指定优先。'
      : ''
  ].filter(Boolean).join('\n');
  const immutableBaseDoorInstruction = [
    ...
    '允许变化项只限于已上传参考图或客户文字明确要求的对象：包边层只改包边/门套线/收口条/压线区域；颜色层默认统一整门可见门面固有色和必要材质观感，但不得覆盖补充要求或独立部件参考图指定的不同颜色；背景层只改背景或抠图白底；把手层只改把手区域。',
  ...
  const doorIdentityLockInstruction = [
    ...
    '包边参考图只约束包边/门套线/收口条/压线区域；颜色参考图默认约束整门可见门面颜色、纹理色差和材质观感；门把手细节图只约束门把手区域；背景文字只约束背景或抠图。',
