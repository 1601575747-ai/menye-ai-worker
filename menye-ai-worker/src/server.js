  ...
  switch (slotId) {
    ...
    case 'glass-grille-detail':
      return [
        ...
        '如果参考图包含整扇门，只能提取气窗/透光窗区域，不能迁移门板主体、把手、锁体、包边、背景或整门款式。',
        'applyDescription 必须说明气窗应如何融合到第一张整门图对应区域，并强调不得改变门扇外轮廓、门型比例和未点名结构。',
        '不要解释，不要输出 markdown。'
      ].join('\n');
    case 'texture-reference':
      return [
...
function isHandleIntegratedLockStyle(style) {
  ...
  return !!(style.referenceContainsHandle && style.hasSmartLockPanel && /把手|门把手|拉手|长拉手|长条|长杆|扶手|手柄|面板/.test(text));
}
  ...
  return styles.map((style) => (
    style.slotId === 'edge-trim-detail' && !useEdgeTrimReferenceColor
      ...
      : style.slotId === 'glass-grille-detail'
        ? `系统识别到${style.label || style.part || '气窗参考图'}特征：来源类型=${style.sourceType || '未识别'}；气窗颜色=${style.color || '未识别'}；程序取样色=${describeSampledColor(style.sampledColor) || '未取到'}；材质=${style.material || '未识别'}；表面质感=${style.finish || '未识别'}；轮廓/形态=${style.shape || '未识别'}；结构=${style.structure || '未识别'}；截面/层次=${style.profile || '未识别'}；边框/收边=${style.edge || '未识别'}；关键细节=${style.details || '未识别'}；执行描述=${style.applyDescription || '只应用到气窗/透光窗/气窗/镂空及其收边区域，不作为门体、包边、把手、锁体或背景参考'}。`
...
function buildDoorImageInstruction(job, maskBox, handleStyle, referenceStyles) {
  ...
  const colorSampleStrictInstruction = hasColorSample
    ? [
        ...
        edgeTrimColorProtectedFromColorSample
          ...
          : (colorSampleUsesReferenceTexture
            ? '颜色任务默认允许统一调整整门可见门面颜色、目标色卡纹理色差和必要材质观感，包括门扇、压线、同门体侧边和包边/门套等可见门面区域；颜色任务本身不得改变门型结构、门板线条数量、线条位置、凹凸深浅、玻璃、把手、包边和门框比例；如本次同时上传门板造型或气窗参考图，对应变化只由对应参考图任务控制。'
            : '颜色任务默认允许统一调整整门可见门面颜色、色偏、明度和饱和度，包括门扇、压线、同门体侧边和包边/门套等可见门面区域；不要强制迁移颜色参考图纹理；颜色任务本身不得改变门型结构、门板线条数量、线条位置、凹凸深浅、玻璃、把手、包边和门框比例；如本次同时上传门板造型或气窗参考图，对应变化只由对应参考图任务控制。'),
        allowBackgroundChange
  ...
  const singleDoorAdditionalPartInstruction = [
    hasLockDetail
      ? [
          ...
          lockReferenceIsHandleIntegrated
            ? `系统识别到该智能锁与把手为物理一体式结构，因此本次锁体任务必须把智能锁面板和一体式把手作为同一个不可拆整体替换到整门图上；把手数量、长度、宽窄、颜色和面板位置应随一体式锁具一起保持一致，不能保留整门图里的原智能锁/原把手。小圆孔是独立实体应急锁孔，不属于把手本体，不能放在把手或黑色智能面板上。${lockReferenceIsDoubleHandle ? '参考图为双把手结构，必须在最终图中保留两根对应把手及其相对位置；不能减少成单把手。' : ''}`
    ...
    hasGlassGrilleDetail
      ? [
          ...
          '如果参考图中包含整门，只能提取气窗/透光窗区域，不迁移门板主体、把手、锁体、包边、颜色、背景或整门款式。',
          '气窗修改不得改变第一张整门图的外轮廓、门型比例、未点名线条结构和包边关系；没有气窗位置时，应在不破坏门型的前提下保守融合。'
        ].join('\n')
      : '',
    hasTextureReference
      ? [
  ...
  const additionalPartConflictResolutionInstruction = [
    hasPanelStyleDetail
      ...
      : '',
    hasGlassGrilleDetail
      ? '冲突消解：本提示词中任何“保持玻璃、镂空或格栅原样”的规则，都只约束气窗参考图以外的任务；客户已上传气窗参考图时，玻璃、格栅、镂空、透光窗及其收边允许按该参考图局部调整，但不能改变门扇外轮廓、比例、包边、把手、锁体、无关门板线条和背景。'
      : '',
    hasTextureReference
    ...
    hasLockDetail
      ? (lockReferenceIsHandleIntegrated
        ? '冲突消解：锁体/智能锁参考图被识别为把手一体式智能锁时，只额外打开“一体式把手锁整体”变化权限；不允许因锁具参考图改变门扇颜色、门板线条、包边、玻璃或背景。'
        : '冲突消解：锁体/智能锁参考图只打开锁具五金局部变化权限，不允许因锁具参考图改变门扇颜色、门板线条、包边、玻璃、把手位置、把手款式或背景；参考图里的把手默认不迁移。')
      : '',
    hasBackgroundReference
  ...
  const immutableBaseDoorInstruction = [
    ...
    hasPanelStyleDetail || hasGlassGrilleDetail
      ? '基础冻结项：门扇外轮廓、宽高比例、透视角度、开门方向、把手安装位置、锁体位置、门框和门扇相对比例必须保持第一张整门图；门板线条/凹凸/门芯造型只有在上传了门板线条/造型参考图时才允许按该参考图局部调整；玻璃/镂空/格栅只有在上传了气窗参考图时才允许按该参考图局部调整。'
      : '绝对冻结项：门扇外轮廓、宽高比例、透视角度、开门方向、门板分割数量、每条装饰线/压线的位置、门芯造型、凹凸深浅、玻璃/镂空位置、把手安装位置、锁体位置、门框和门扇相对比例。',
    lockReferenceIsHandleIntegrated
      ? '例外说明：本次锁体参考图已识别为把手一体式智能锁，因此“把手安装位置/把手形态冻结”只对非一体式把手区域生效；一体式智能锁所在把手区域必须随锁具整体替换、迁移和对齐，不能保留原把手锁。'
      : '',
    allowDoorSurfaceColorChange
      ? (edgeTrimColorProtectedFromColorSample
        ? '允许变化项只限于已上传参考图、背景信息或客户文字明确要求的对象：包边层只改包边/门套线/收口条/压线区域，并按客户语义判断包边独立颜色；颜色层按颜色参考图调整门扇/门体可见表面，但不得覆盖包边独立颜色；背景层只在上传背景参考图、填写背景信息或要求抠图/白底时改背景；把手层只改把手区域；锁体层、门板造型层、气窗层、材质纹理层仅在上传对应参考图时改对应区域。'
        : '允许变化项只限于已上传参考图、背景信息或客户文字明确要求的对象：包边层只改包边/门套线/收口条/压线区域；颜色层按颜色参考图统一调整整门可见门面，默认覆盖包边并让包边跟门体同色；只有客户明确要求包边独立颜色或按包边参考图颜色时，包边颜色才不参与统一；背景层只在上传背景参考图、填写背景信息或要求抠图/白底时改背景；把手层只改把手区域；锁体层、门板造型层、气窗层、材质纹理层仅在上传对应参考图时改对应区域。')
      : hasBackgroundReference
        ? '允许变化项只限于已上传参考图、背景信息或客户文字明确要求的对象：背景层必须把第一张整门图的门体抠出并贴入背景参考图目标门位；门体本身不得改色、重画或换款，只允许整体缩放、透视拉伸、旋转、轻微裁切、边缘融合和接地阴影；把手层、包边层、锁体层、门板造型层、气窗层、材质纹理层仅在上传对应参考图时改对应区域。'
        : '允许变化项只限于已上传参考图、背景信息或客户文字明确要求的对象：包边层只改包边/门套线/收口条/压线区域，并让包边颜色匹配第一张整门图的原门体颜色；本次没有颜色层，门扇/门体颜色不得改变；背景层只在上传背景参考图、填写背景信息或要求抠图/白底时改背景；把手层只改把手区域；锁体层、门板造型层、气窗层、材质纹理层仅在上传对应参考图时改对应区域。',
    freezeDoorSurfaceColor ? '门体颜色冻结项：未上传颜色参考图且未明确要求改门体颜色时，门扇/门体颜色不属于允许变化项；包边同门同色时，只能改包边颜色去匹配第一张整门图的原门体颜色，不能反向改变门体颜色。' : '',
    '如果参考图中的包边、颜色、把手、锁体、门板造型、气窗、材质纹理或背景与第一张整门图的基础门型冲突，必须优先保留第一张整门图的门体外轮廓、比例、透视和门框关系；只有对应目标区域允许局部融合，不能扩展成整门重画。',
    hasPanelStyleDetail || hasGlassGrilleDetail
      ? '失败结果定义：除门板线条/造型参考图和气窗参考图明确对应的目标区域外，最终图只要出现新的门扇比例、新的把手位置、新的锁体位置、无关区域的新门板线条、无关区域的新玻璃位置、或变成另一扇浅色/深色商品门，就属于失败，必须改回第一张整门图的基础门型。'
  ...
  const doorIdentityLockInstruction = [
    ...
    lockReferenceIsHandleIntegrated
      ? `把手一体式智能锁例外：只允许在一体式智能锁把手自身区域内替换把手和锁面板的整体位置、数量、长度、宽窄、颜色、黑色面板和安装衔接；小圆孔作为独立实体应急锁孔，不属于把手本体，不能放在把手或黑色智能面板上；${lockReferenceIsDoubleHandle ? '参考图是双把手时，最终必须保留双把手数量、左右/双扇相对位置和长短比例，不能生成单把手。' : ''}如果整门图中已有隐蔽智能锁、小圆孔或旧锁面板，必须被新的一体式智能锁覆盖或清理，不得叠加残留。门型、包边、门板线条和背景仍冻结。`
      : '',
    edgeTrimColorProtectedFromColorSample
      ...
      : '包边参考图只约束包边/门套线/收口条/压线区域的结构、宽窄、层次、线条和收边方式；颜色参考图默认约束整门可见门面颜色、纹理色差和材质观感，包含包边同色；门把手细节图只约束门把手区域；锁体/智能锁参考图只约束锁具五金区域；门板造型参考图只约束门扇表面线条/凹凸/门芯造型；气窗参考图只约束气窗/透光窗/气窗/镂空区域；材质纹理参考图只约束门体表面纹理和质感；背景参考图和背景文字只约束背景、墙面、地面、空间和光线或抠图。',
    ...
    !hasHandleDetail && !hasEdgeTrimDetail && !hasColorSample && !hasLockDetail && !hasPanelStyleDetail && !hasGlassGrilleDetail && !hasTextureReference && !hasBackgroundReference
      ...
      : !hasHandleDetail
        ? (lockReferenceIsHandleIntegrated
          ...
          : '当前没有门把手细节照，请保持整门图中的现有门把手，不要擅自改变门把手款式、颜色、材质或底座；本次应优先执行已上传参考图对应的包边、颜色、锁体、门板造型、气窗、材质纹理或背景任务。')
        : [
