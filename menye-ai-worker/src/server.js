...
function buildDoorImageInstruction(job, maskBox, handleStyle, referenceStyles) {
  ...
  const immutableBaseDoorInstruction = [
    ...
    freezeDoorSurfaceColor ? '门体颜色冻结项：未上传颜色参考图且未明确要求改门体颜色时，门扇/门体颜色不属于允许变化项；气窗参考图、包边参考图、门板造型图、锁体图、把手图、材质纹理图和背景图都不能作为门体改色来源；包边同门同色时，只能改包边颜色去匹配第一张整门图的原门体颜色，不能反向改变门体颜色。' : '',
    '如果参考图中的包边、颜色、把手、锁体、门板造型、气窗、材质纹理或背景与第一张整门图的基础门型冲突，必须优先保留第一张整门图的门体外轮廓、比例、透视和门框关系；只有对应目标区域允许局部融合，不能扩展成整门重画。',
    hasPanelStyleDetail || hasGlassGrilleDetail
      ? '失败结果定义：除门板线条/造型参考图和气窗参考图明确对应的目标区域外，最终图只要出现新的门扇比例、新的把手位置、新的锁体位置、无关区域的新门板线条、无关区域的新玻璃位置、门体颜色被气窗参考图带偏、或变成另一扇浅色/深色商品门，就属于失败，必须改回第一张整门图的基础门型和原门体颜色。'
  ...
  const doorIdentityLockInstruction = [
    ...
    lockReferenceIsHandleIntegrated
      ...
      : '',
    edgeTrimColorProtectedFromColorSample
      ? '包边参考图只约束包边/门套线/收口条/压线区域的结构、宽窄、层次、线条和收边方式；颜色参考图约束门扇/门体可见表面颜色、纹理色差和材质观感，但本次包边颜色按客户独立颜色意图执行；门把手细节图只约束门把手区域；锁体/智能锁参考图只约束锁具五金区域；门板造型参考图只约束门扇表面线条/凹凸/门芯造型；气窗参考图只约束气窗/透光窗/玻璃/格栅/镂空区域，气窗颜色只作用于该局部区域；材质纹理参考图只约束门体表面纹理和质感；背景参考图和背景文字只约束背景、墙面、地面、空间和光线或抠图。'
      : '包边参考图只约束包边/门套线/收口条/压线区域的结构、宽窄、层次、线条和收边方式；颜色参考图默认约束整门可见门面颜色、纹理色差和材质观感，包含包边同色；门把手细节图只约束门把手区域；锁体/智能锁参考图只约束锁具五金区域；门板造型参考图只约束门扇表面线条/凹凸/门芯造型；气窗参考图只约束气窗/透光窗/玻璃/格栅/镂空区域，气窗颜色只作用于该局部区域；材质纹理参考图只约束门体表面纹理和质感；背景参考图和背景文字只约束背景、墙面、地面、空间和光线或抠图。',
    freezeDoorSurfaceColor ? '没有颜色参考图或明确改门色要求时，必须保留第一张整门图的门体原始颜色；气窗参考图和包边参考图都不能作为门体颜色来源。' : '',
    hasPanelStyleDetail || hasGlassGrilleDetail
