...
function buildDoorImageInstruction(job, maskBox, handleStyle, referenceStyles, dimensionBoxes) {
  ...
  const backgroundLockPolicyText = hasLockDetail
    ? (userExplicitlyPreservesHandles
      ? '锁体/智能锁按 lock-detail 参考图执行替换/融合，但客户已明确保留原把手/拉手，因此只能迁移智能锁面板、锁芯、密码/指纹区、应急小圆孔和必要安装底座，不得改变、截断、移动或删除原把手/拉手'
  ...
  const lockDetailMustApplyInstruction = hasLockDetail
    ? [
        ...
        userExplicitlyPreservesHandles
          ...
          : '',
        userExplicitlyPreservesHandles
          ? '保留把手优先级：客户明确保留把手/拉手时，原把手/双拉手本体是冻结对象。如果智能锁与把手空间冲突，必须缩小智能锁面板、侧移到把手左侧/右侧/上方的空白五金区，或只保留锁面板核心特征；不得覆盖、截断、分裂、缩短、移动、改色或重画任何一根原把手/拉手。'
        ...
        userExplicitlyPreservesHandles
          ? '双开门保留拉手规则：如果客户要求保留双拉手，最终左右两根原拉手必须保持完整且成对，金色竖拉手必须是连续完整的长条；智能锁只能贴近中缝但避开拉手外轮廓，放在拉手侧边、上方或原锁孔区域，不得压住拉手任何一段，不得把任一拉手变短、断开、下移、错位或改成半截。'
  ...
  const requiredReferenceTasks = [
    ...
    hasLockDetail ? (lockReferenceCanReplaceHandle
      ...
      : (userExplicitlyPreservesHandles
        ? '锁体/智能锁：客户明确要求保留原把手/拉手，因此只迁移智能锁面板、密码/指纹/刷卡区、锁芯孔、小圆孔和必要安装底座；不得迁移参考图中的一体式把手本体，不得截断、移动、删除或缩短原把手/拉手'
    ...
    hasLockDetail
      ? [
          ...
          lockReferenceCanReplaceHandle
            ...
            : (userExplicitlyPreservesHandles
              ? '客户已明确保留把手/拉手，因此即使锁体参考图被识别为把手一体式智能锁，也只能提取智能锁面板、密码/指纹/刷卡区、锁芯孔、小圆孔和必要底座；参考图中的把手本体不得迁移。新锁必须避让原把手/双拉手，不能与拉手外轮廓重叠；宁可缩小锁面板或放到拉手旁边的空白五金区，也必须保留原把手/双拉手的完整长度、位置、数量和形状。'
  ...
  const additionalPartConflictResolutionInstruction = [
    ...
    hasLockDetail
      ? (lockReferenceCanReplaceHandle
        ...
        : (userExplicitlyPreservesHandles
          ? '冲突消解：客户明确保留把手/拉手时，锁体/智能锁参考图只打开锁具五金局部变化权限；即使参考图是一体式把手锁，也不得迁移把手本体，不得覆盖、截断、移动、缩短、分裂或删除原把手/双拉手。若智能锁与拉手重叠，必须调整智能锁的位置或尺寸，不能调整拉手。'
    ...
    !hasHandleDetail && !hasEdgeTrimDetail && !hasColorSample && !hasLockDetail && !hasPanelStyleDetail && !hasGlassGrilleDetail && !hasHeaderColumnDetail && !hasTextureReference && !hasBackgroundReference
      ...
      : !hasHandleDetail
        ? (lockReferenceCanReplaceHandle
          ...
          : (userExplicitlyPreservesHandles
            ? '当前没有门把手细节照，且客户明确要求保留把手/拉手；请完整保留整门图中的现有把手/拉手数量、长度、位置、形状、颜色和底座。lock-detail 只能处理智能锁面板、锁芯孔、小圆孔和必要安装底座，不得把参考图中的把手本体迁移过来，也不得让新智能锁遮挡、截断或缩短任何原把手/拉手。'
